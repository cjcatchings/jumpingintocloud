...
export default function EcrSummary(){
    ...
    return (
        ...
        <ul className="list-disc pt-2">
            <li className="ml-6">Middleware:  Runs a Python Flask container that executes application logic such as authentication, data retrieval (roster, players) and 
            ...
            <li className="ml-6">Frontend:  A NextJS application container that runs on a NodeJS server.  This performs server-side rendering (SSR) to deliver content to web 
        ...
        <p className="pt-2">The data layer runs on MongoDB&apos;s free cloud offering called Atlas.  I initially attempted to run MongoDB as a separate container with the v7.0.5 image 
        ...
        <p className="pt-2">I first created two public repositories in ECR - one for the middleware image and another for the frontend image.  I considered creating a private repo but I do not mind sharing these 
        ...
        <p className="pt-2">Next step was to log into my public ECR repo.  I had already built the Docker images for my application (details to come).</p>
        <p className="pt-2">In order to push them to ECR, I had to 
        ...
        <p className="pt-2">If you have an AWS profile configured (whether via access key and secret or logging in via Identity Center), a prompt saying &quot;Login successful&quot; should be displayed.
...
elvira/fantasyfootball-mw        v0.1      654321fedcba   3 days ago      205MB`}
        ...
        <p className="pt-2">The <code>IMAGE ID</code> value is what you need to pass into the following command to &quot;tag&quot; the image for your ECR repo.</p>
        <pre className="mx-6 bg-slate-300 w-9/12 text-wrap mt-1 pl-1"><code className="text-xs pl-1">
{`>>> docker tag <imageID> public.ecr.aws/<registry_alias>/<app_name>`}
        </code></pre>
        <p className="pt-2">Remember that each repository created included a registry alias and an app name.  In my case, the paths were as follows:</p>
        <ul className="list-disc pt-2">
        ...
        <p className="pt-2">The following command was used to tag the images prior to pushing to ECR:</p>
        <pre className="mx-6 bg-slate-300 w-9/12 text-wrap mt-1 pl-1"><code className="text-xs pl-1">
...
>>> docker tag elvira/fantasyfootball-mw:v0.1 elvira/fantasyfootball-mw`}
        ...
        <p className="pt-2">Having tagged the images in their respective ECR repositories, I could now push them with the following command:</p>
        <pre className="mx-6 bg-slate-300 w-9/12 text-wrap mt-1 pl-1"><code className="text-xs pl-1">
