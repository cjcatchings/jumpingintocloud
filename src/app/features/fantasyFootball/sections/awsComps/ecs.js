        ...
        <TextSubSection
            ...
            <TextSubSection
                ...
                <ul className="list-disc pt-2">
                    <li className="ml-6 mr-2"><span className="font-bold">EC2</span> – this allows you to configure and provision your own EC2 instances that will run your containers.
                    ...
                    <li className="ml-6 mr-2"><span className="font-bold">Fargate</span> - this option reduces the management over head by requiring you to simply configure the number of vCPUs, memory and operating system for your container instances.
                ...
                <p className="pt-2 mr-2">ECS comes with its own vocabulary for its building blocks, though most in the IT field will be familiar with the terms:</p>
                <ul className="list-disc pt-2">
                    <li className="ml-6 mr-2"><span className="font-bold">Cluster</span> – The root building block of ECS.  This will define the tasks and services running your containers as well as define the infrastructure running them (capacity providers and container instances)
                    </li>
                    <li className="ml-6 mr-2"><span className="font-bold">Task Definition</span> – A specified configuration for how your application is deployed on ECS.  It defines the images (ECR or DockerHub) for the containers that run your application, how much vCPU/memory is configured for your
                    ...
                    <li className="ml-6 mr-2"><span className="font-bold">Task</span> – A container or set of containers launched from a task definition that typically execute once, stop on their own and are not replaced automatically.  These are best for one-off or batch jobs that run on a schedule.
                    </li>
                    <li className="ml-6 mr-2"><span className="font-bold">Service</span> – One or more tasks that run indefinitely and are replaced automatically when it becomes ‘unhealthy’.  An ECS service will ensure that a configured number of tasks are running at any given time and will replace failed tasks.  These are typically used for long-running processes such as web applications that need to be available to serve requests.
                    </li>
                </ul>
            </TextSubSection>
            <TextSubSection
                ...
                <ul className="list-disc pt-2">
                    <li className="ml-6 mr-2"><span className="font-bold">Instance Role:</span>  This role will be attached to the EC2 instance (as part of the instance profile) running the task containers and needs access to resources and services critical to operating ECS such as
                    ...
                    <li className="ml-6 mr-2"><span className="font-bold">Task Execution Role:</span>  This role will be assumed by the container agent running on the EC2 instance and will make calls to AWS services on your behalf in order to manage the containers on the instance.  </li>
                    <li className="ml-6 mr-2"><span className="font-bold">Task IAM Role (optional):</span>  This role can be assigned to your containers to access any external AWS services (S3, DynamoDB etc.) required to fulfill task execution.   </li>
                </ul>
                <p className="pt-2 mr-2">For the instance role, I attached the following AWS-managed policies:</p>
                <ul className="list-disc pt-2">
                    <li className="ml-6 mr-2">AmazonEC2ContainerServiceForEC2Role (not needed if you’re using the Fargate launch type)</li>
                    <li className="ml-6 mr-2">AmazonEC2ContainerServiceRole</li>
                </ul>
                <p className="pt-2 mr-2">I also added the following trust relationship policy:</p>
    ...
    "Statement": [
        {
            ...
            "Action": "sts:AssumeRole"
                ...
                <p className="pt-2 mr-2">For the task execution role, AWS recommends the following permissions policy:</p>
                <pre className="mx-6 bg-slate-300 w-9/12 text-wrap mt-1 pl-1"><code className="text-xs pl-1">
                    {`{
  "Version": "2012-10-17",
  "Statement": [
    {
      ...
      "Resource": "*"
                ...
                <p className="pt-2 mr-2">This role also needs the following trust relationship policy so that the ECS tasks service can assume it on your behalf:</p>
                <pre className="mx-6 bg-slate-300 w-9/12 text-wrap mt-1 pl-1"><code className="text-xs pl-1">
                    {`{
  "Version": "2012-10-17",
  "Statement": [
    {
      ...
      "Action": "sts:AssumeRole"
                ...
                <p className="pt-2 mr-2">At the moment, this application does not require a task IAM role.  However, if your ECS task needs access to AWS resources, 
   ...
   "Statement":[
      {
         ...
         "Condition":{
            ...
            <TextSubSection
                ...
                <p className="pt-2 mr-2">I created a key-pair (so I could troubleshoot issues by connecting via SSH to the instance.  In a production scenario I would use AWS Systems Manager Session Manager to connect to EC2 instances without opening port 22 for SSH). </p>
                <p className="pt-2 mr-2">I specified the t2.micro instance type for my launch template and provided the desired subnets (the public subnets for each availability zone in VPC – eu-central-1a, eu-central-1b, eu-central-1c) in which to create my EC2 instances and 
                the security groups to assign them to.  I opened the following ports for my EC2 instances via security groups:</p>
                <ul className="list-disc pt-2">
                    <li className="ml-6 mr-2">Port 3000 to my VPC CIDR block (172.31.0.0/16) so that an ELB application load balancer can access my frontend (a NextJS app running on port 3000)</li>
                    <li className="ml-6 mr-2">Port 5000 to my 3 private subnets so that my NextJS frontend app can access my application layer (a Python Flask app running on port 5000)</li>
                    <ul className="list-disc pt-2">
                        <li className="ml-6 mr-2">172.31.0.0/20</li>
                        <li className="ml-6 mr-2">172.31.16.0/20</li>
                        <li className="ml-6 mr-2">172.31.32.0/20</li>
                    </ul>
                    <li className="ml-6 mr-2">Port 22 to my local IP address for troubleshooting purposes.  In a true production environment this would not be desired.</li>
                </ul>
                <p className="pt-2 mr-2">I operate this application out of public subnets since my application requires access to MongoDB Atlas.  In a true production environment, I would likely do the following:</p>
                <ul className="list-disc pt-2">
                    <li className="ml-6 mr-2">Establish a VPC peering relationship with my MongoDB Atlas cluster (running on an M10 cluster type) and update the necessary route tables to route to this cluster OR migrate the database to Amazon DynamoDB and use a VPC 
                    endpoint to access the database inside the AWS network (no public internet exposure).</li>
                    <li className="ml-6 mr-2">Specify my private subnets for the EC2 container instances in my launch template.</li>
                    <li className="ml-6 mr-2">Use  an instance type larger than t2.micro (or use the Fargate launch type).</li>
                </ul>
                <p className="pt-2 mr-2">Lastly, I set the container instance role to that defined above and provided a user data script that will inform ECS to assign the EC2 instance to my ECS cluster:</p>
                <pre className="mx-6 bg-slate-300 w-9/12 text-wrap mt-1 pl-1"><code className="text-xs pl-1">
{`#!/bin/bash
echo "ECS_CLUSTER=MyECSClusterName" >> /etc/ecs/ecs.config`}                    
            ...
            <TextSubSection
                ...
                <p className="pt-2 mr-2">In order to execute Python code that can provision resources in my AWS account, I had to establish a profile in AWS Identity Center.  Details for that setup will be provided in the future (as a separate link).  However, at a high level, 
                ...
                <p className="pt-2 mr-2">Once this was created, an Identity Center URL was provided to log in as a user with a certain role attached.  I created this profile in my ~/.aws/config file so that my PyCharm environment could detect this profile.  The profile definition had the following:</p>
                <pre className="mx-6 bg-slate-300 w-9/12 text-wrap mt-1 pl-1"><code className="text-xs pl-1">
...
output = json`}    
                ...
                <p className="pt-2 mr-2">From a command line, I ran the following command to begin the authentication process from my AWS Identity Center profile:</p>
                <pre className="mx-6 bg-slate-300 w-9/12 text-wrap mt-1 pl-1"><code className="text-xs pl-1">
{`>>> aws sso login –proflie myDevProfile`}    
                </code></pre>
                <p className="pt-2 mr-2">This opened a browser with the above sso_start_url where I could log in as my developer user.  Once logged in, I could return to PyCharm and execute Python commands as this Identity Center user.</p>
            </TextSubSection>
            <TextSubSection
                sectionInitiallyVisible={false}
                sectionType="h2"
...
new_ecs_cluster = ecs_client.create_cluster(
	clusterName=’FantasyFootballCluster’
                ...
                <p className="pt-2 mr-2">This created the cluster without having to create an auto scaling group.  As mentioned previously, in a production environment, I would include an auto scaling group to ensure the number of resources required to scale my application were available.</p>
           </TextSubSection>
           <TextSubSection
                sectionInitiallyVisible={false}
                sectionType="h2"
                ...
                <p className="pt-2 mr-2">I created two log groups:</p>
                <ul className="list-disc pt-2">
                    <li className="ml-6 mr-2">/ecs/CJCFantasyFootballMW – Stores the Flask logs for my application layer</li>
                    <li className="ml-6 mr-2">/ecs/CJCFantasyFootballFE – Stores the NextJS logs for my frontend layer</li>
                </ul>
            </TextSubSection>
            <TextSubSection
                sectionInitiallyVisible={false}
...
{`task_def = ecs_client.register_task_definition(
    ...
    requiresCompatibilities=['EC2']
                ...
                <p className="pt-2 mr-2">A few notes on how I configured this task definition:</p>
                <ul className="list-disc pt-2">
                    <li className="ml-6 mr-2">I used the `host` network mode as my containers run in public subnets in order to access MongoDB Atlas while remaining in free tier.  In a production environment, I would likely 
                    ...
                    <li className="ml-6 mr-2">I specified environment variables required to connect to my MongoDB Atlas database, encode/decode JWT tokens and generate password hashes in the task definition itself.  
...
{`service_resp = ecs_client.create_service(
    ...
    schedulingStrategy='REPLICA'
                ...
                <p className="pt-2 mr-2">Had my task definition used the ‘awsvpc’ network mode, I would provide subnets and security groups for my service.  In a production environment, these would be my private subnets and security groups restricting port access only to those
